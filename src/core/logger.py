...
class Logger:
    ...
    def _setup_logger(cls):
        ...
        if cls._logger is None:
            ...
            # ПРОВЕРКА: Как запущен плагин?
            if getattr(sys, 'frozen', False):
                # Если плагин скомпилирован в .exe (через PyInstaller)
                # sys.executable вернет путь к файлу MyPlugin.exe.
                # os.path.dirname от него — это папка, где лежит этот .exe.
                # И внутри создается папка 'logs'
                base_path = os.path.join(os.path.dirname(sys.executable), 'logs')
            else:
                # Если мы запускаем плагин в режиме разработки (как .py скрипт)
                # __file__ — это путь к текущему файлу (то есть к .../core/logger.py)
                # Три раза dirname поднимают нас по дереву папок вверх:
                # 1-й: папка 'core'
                # 2-й: папка 'src'
                # 3-й: корневая папка плагина
                # И там создается папка 'logs'
            ...
            try:
                ...
                # Итоговый путь к файлу: .../logs/plugin.log
